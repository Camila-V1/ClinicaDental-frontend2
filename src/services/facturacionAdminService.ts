...
class FacturacionAdminService {
  ...
  async getFactura(id: number) {
    const response = await api.get<Factura>(`/api/facturacion/api/facturas/${id}/`);
    return response.data;
  }

  async createFactura(data: FacturaCreateData) {
    const response = await api.post<Factura>('/api/facturacion/api/facturas/', data);
    return response.data;
  }

  async updateFactura(id: number, data: Partial<FacturaCreateData>) {
    const response = await api.put<Factura>(`/api/facturacion/api/facturas/${id}/`, data);
    return response.data;
  }

  async deleteFactura(id: number) {
    await api.delete(`/api/facturacion/api/facturas/${id}/`);
  }

  async marcarPagada(id: number) {
    const response = await api.post<Factura>(`/api/facturacion/api/facturas/${id}/marcar-pagada/`);
    return response.data;
  }

  async cancelarFactura(id: number) {
    const response = await api.post<Factura>(`/api/facturacion/api/facturas/${id}/cancelar/`);
    return response.data;
  }
  ...
  async getPago(id: number) {
    const response = await api.get<Pago>(`/api/facturacion/api/pagos/${id}/`);
    return response.data;
  }

  async createPago(data: PagoCreateData) {
    const response = await api.post<Pago>('/api/facturacion/api/pagos/', data);
    return response.data;
  }

  async updatePago(id: number, data: Partial<PagoCreateData>) {
    const response = await api.put<Pago>(`/api/facturacion/api/pagos/${id}/`, data);
    return response.data;
  }

  async deletePago(id: number) {
    await api.delete(`/api/facturacion/api/pagos/${id}/`);
  }

  async anularPago(id: number) {
    const response = await api.post<Pago>(`/api/facturacion/api/pagos/${id}/anular/`);
    return response.data;
  }

  async getPagosPorFactura(facturaId: number) {
    const response = await api.get<Pago[]>('/api/facturacion/api/pagos/por-factura/', {
